...
//
// Every handler checks `req.hasInstagram` (set by AuthGuard from
// `InfluencerContext.hasInstagram`) before calling the Instagram
// Graph API. This makes the intent explicit and works for all auth
// providers: instagram, google, password, and any future additions.
...
/** True iff this session has a connected Instagram account. */
const hasIG = (req: Request): boolean => !!(req as any).hasInstagram;
...
export class SocialAccountsController {
  ...
  async getProfile(@Req() req: Request) {
    if (!hasIG(req)) {
      return this.socialAccountsService.getProfileFromDb((req as any).influencerId);
    }
  ...
  // ── Account insights — empty for non-Instagram providers ─────────────

  @Get('api/insights/overview')
  async getOverview(@Req() req: Request) {
    if (!hasIG(req)) return { data: { results: [] } };
